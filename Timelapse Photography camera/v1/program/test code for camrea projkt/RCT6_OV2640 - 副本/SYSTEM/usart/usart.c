...
void USART2SendStr(char* str)
{
	...
	while( USART_GetFlagStatus(USART2, USART_FLAG_TC) == RESET ); //判断 str 是否已被（全部！！！）发送出去
}
...
//    DMA_InitStruct.DMA_MemoryBaseAddr =(uint32_t)jpeg_buf; // 发送内存地址，从哪里发送
...
void uart2_init(u32 bound){
  ...
  USART_Cmd(USART2, ENABLE);                    //使能串口1 
   ...
   /* 串口DMA配置 */
    DMA_DeInit(DMA1_Channel7);  // DMA1 通道7，寄存器复位
    // RX DMA1 通道4
    DMA_InitStruct.DMA_M2M = DMA_M2M_Disable;                   // 本次是外设到内存，所以关闭内存到内存
    DMA_InitStruct.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;// 内存数据宽度，按照字节存储
    DMA_InitStruct.DMA_Mode = DMA_Mode_Normal;                  // 正常模式
    ...
    USART_DMACmd(USART2, USART_DMAReq_Tx, ENABLE);// 使能DMA串口发送和接受请求
